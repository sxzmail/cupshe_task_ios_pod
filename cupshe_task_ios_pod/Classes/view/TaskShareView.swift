...
//        self.taskMaskView.addGestureRecognizer(UIGestureRecognizer.init(target: self, action: #selector(dismissTaskList)))
        ...
        self.popContentView = UIView(frame: CGRect(x: 0, y: screenHeight, width: screenWidth, height: (151 + 52 + 40) * heightPercent))
...
//        popTitleTxt.font = UIFont.systemFont(ofSize: 16 * heightPercent,weight: .bold)//UIFont(name:, size: 16 * heightPercent)
        ...
        var row =  UIView(frame: CGRect(x: (screenWidth * 0.05), y:  52 * heightPercent, width: (screenWidth * 0.9), height: 151 * heightPercent))
...
//        colView_twitter.addSubview(twitterLbl)
        ...
        var bottomSpace =  UIView(frame: CGRect(x: 0, y:  CGFloat(151 + 52) * heightPercent, width: (screenWidth), height: 40 * heightPercent))
